...
def turnover_figure(bpath, nsp):
    ...
    lifetime_distribution_loglog(axs['1,1'], bpath, nsp, bins=50, discard_t=0.,
                                 with_starters=True)

    lifetime_distribution_loglog_linear_bins(axs['2,1'], bpath, nsp,
                                             bin_w=10*ms, discard_t=0.,
                                             with_starters=True)

    lifetime_distribution_loglog_linear_bins(axs['3,1'], bpath, nsp,
                                             bin_w=100*ms, discard_t=0.,
                                             with_starters=True)

    lifetime_distribution_loglog_linear_bins(axs['4,1'], bpath, nsp,
                                             bin_w=1000*ms, discard_t=0.,
                                             with_starters=True)

    lifetime_distribution_loglog_linear_bins(axs['5,1'], bpath, nsp,
                                             bin_w=10000*ms, discard_t=0.,
                                             with_starters=True)
    ...
    lifetime_distribution_loglog_linear_bins(axs['5,2'], bpath, nsp,
                                             bin_w=10000*ms, discard_t=0.,
                                             with_starters=False)
